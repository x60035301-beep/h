...
const labels = {
  zh: { dwell: "平均停留", conversion: "成交率", funnel: "漏斗统计", drag: "拖拽更新数据库接口预留", empty: "暂无销售 Pipeline 数据", emptyDescription: "添加真实客户并推进阶段后，这里会自动显示每个销售阶段的客户数量、金额和停留时间。" },
...
export default async function PipelinePage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const maxAmount = Math.max(1, ...pipelineStages.map((stage) => stage.amount));
  ...
  return (
    <div className="page-shell">
      ...
      {pipelineStages.length ? (
        ...
        <EmptyState icon={Database} title={"empty" in page ? page.empty : "No pipeline data"} description={"emptyDescription" in page ? page.emptyDescription : "Add real customers to populate the sales pipeline."} />
      )}
      ...
      <Card>
        ...
        <CardContent className="grid gap-3 lg:grid-cols-2">
          {pipelineStages.length ? (
            ...
            <p className="rounded-md border border-dashed p-6 text-center text-sm text-muted-foreground lg:col-span-2">
              {"empty" in page ? page.empty : "No pipeline data"}
            </p>
          )}
        </CardContent>
      </Card>
